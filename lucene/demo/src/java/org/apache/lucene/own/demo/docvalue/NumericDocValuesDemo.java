...
public class NumericDocValuesDemo {
  ...
  public static List<Document> genDocs(int blockType) {
    ...
    for (int block = 0; block < 10; block++) {
      for (int i = 0; i < 1 << 16; i++) {
        // 制造一种特殊场景：block 0 为空
        if (block == 0) {
          docs.add(new NumericRow(-1, -1, -1).document());
          continue;
        }
        ...
        docs.add(new NumericRow(r.nextInt(100000), r.nextInt(100000), r.nextInt(100000)).document());
      }
    }
    return docs;
  }
